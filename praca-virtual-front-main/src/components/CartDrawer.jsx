// src/components/CartDrawer.jsx — com "Finalizar compra" funcional
...
export default function CartDrawer() {
  ...
  const finalizarPedido = async () => {
    ...
    try {
      // Todos os itens do carrinho pertencem ao mesmo restaurante
  ...
  return (
    <>
      ...
      {/* Painel */}
      <div
        className={`fixed top-0 right-0 h-full z-50 flex flex-col w-[85%] max-w-[400px] bg-[#1E1E1E] transition-transform duration-300 ${
          ...
          {itens.length === 0 ? (
            ...
            itens.map((item) => (
              <div key={item.id} className="flex items-center gap-3 p-3 rounded-2xl bg-[#2A2A2A]">
                <div className="w-16 h-16 rounded-xl flex-shrink-0 bg-[#3B82F6]/20 flex items-center justify-center text-2xl">
                  🍔
                </div>
                ...
                <button onClick={() => removerItem(item.id)} className="text-gray-400 text-xl hover:text-red-400">
                  ✕
                </button>
              </div>
            ))
